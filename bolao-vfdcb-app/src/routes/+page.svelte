<script lang="ts">
    ...
    const userRanking = $derived.by(() => {
        if (!$user) return null;
        const index = $rankingStore.data.findIndex(r => r.user_id === $user.id);
...
<div class="container home-container" style="padding-top: 4rem; padding-bottom: 4rem;">
    ...
    <div class="home-grid animate-fade-in" class:full-width={!$user} style="animation-delay: 0.1s;">
        {#if $user}
            ...
            <aside class="ranking-sidebar">
                <div class="card" style="padding: 1.5rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                        ...
                        <a href="/ranking" style="font-size: 0.875rem; color: var(--color-primary); text-decoration: none; font-weight: 500;">Ver tudo</a>
                    </div>
                    ...
                    {:else}
                        <div class="mini-ranking">
                            {#each topRankings as rank, i}
                                <div class="ranking-item {rank.user_id === $user?.id ? 'active' : ''}">
                                    <div class="rank-num">{getRankEmoji(i)}</div>
                                    <div class="rank-name">{rank.user_name}</div>
                                    <div class="rank-pts">{rank.total_score} pts</div>
                                </div>
                            {/each}
                            ...
                            {#if userRanking && !isUserInTop3}
                                ...
                                <div class="ranking-item active">
                                    <div class="rank-num" style="font-size: 0.75rem; color: var(--color-text-muted);">{userRanking.rank}º</div>
                                    <div class="rank-name">{userRanking.user_name}</div>
                                    <div class="rank-pts">{userRanking.total_score} pts</div>
                                </div>
                            {/if}
                        </div>
                    {/if}
...
<style>
    ...
    @media (min-width: 1024px) {
        .home-grid {
            grid-template-columns: 1fr auto 2fr; /* 1/3, separator, and 2/3 distribution */
        }
